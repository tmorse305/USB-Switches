...
class Controller(udi_interface.Node):
    ...
    def checkParams(self):
        ...
        if self.Parameters["devfile"] is not None:
            print ("devfile:",self.Parameters["devfile"])
            try:
    ...
    def poll(self, flag):
        if 'longPoll' in flag:
            LOGGER.debug('crtl longPoll re-parse updateallfromserver (controller)')
        else:
            self.heartbeat()
            LOGGER.debug('crtl shortPoll check for events (controller)')
